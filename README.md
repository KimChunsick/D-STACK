...
dstack mode exec audit-batch-001 --role audit --context audit-context.md --output audit.md --quick <slug> --writable <dir>
```
...
메인은 사용자 대화, 작업 배분, 짧은 결과 확인과 CLI 상태 기록을 맡아요. 조사·구현·테스트·실패
진단·수정·큰 증거 파일 검사는 새 맥락의 워커가 맡고, 한 줄 수정도 같은 경계를 지켜요.
워커가 실패하거나 도구·실행 자리가 없으면 대기 또는 차단 상태로 남겨요. 다시 시도할 때는
필요한 내용만 새 워커에게 넘겨요. 자세한 로그는 산출물에 두고, 반환에는 위치·HEAD,
R별 결과, 변경 파일·커밋, 명령·종료 코드, 산출물 경로와 미실행 사유를 담아요.

워커가 실행되는 동안에는 호스트가 제공하는 중단 가능한 완료 대기를 사용해요. 질문에 답한
뒤 같은 작업을 기다리고, 추가 요구는 요청·결정 대장에 적은 뒤 영향받는 작업만 조정해요.
충돌하는 변경이나 중지 요청은 해당 워커를 멈추고 실제 상태를 확인해요. 실행 중인 Plan의
수정 제한은 유지하며, 지원하는 CLI 전환이 없으면 대기 또는 차단 사유를 알려 드려요.
중지는 완료로 기록하지 않아요. 호스트가 대기 중 입력을 지원하지 않으면 그 한계를 밝히고,
입력을 받으려고 작업을 중복 실행하거나 완료했다고 보고하지 않아요.

공통 규칙은 [runtime.md](claude/runtime.md)에 있어요. GSD의 새 맥락·가벼운 조정자 원칙은
[기존 출처와 인용문](claude/skills/dstack-develop/SKILL.md#11-sentences-taken-from-gsd-r61)에
연결해 두었어요. 원본 저장소와 커밋은 이번 작업에서 확보하지 못해 기존 기록을 보존한
2차 출처로 표시해요. 이번에 원문을 새로 검증했다는 뜻은 아니에요.
...
- **판정은 명령이 계산해요.** 체크박스를 에이전트가 직접 켜는 일은 없어요. 증거 행은
  ...
  양쪽 메인은 요청·증거 기록과 `dstack gate`를 맡고, 저장소 테스트와 한국어 검사는 새로 부른
  워커가 실행해요. Claude 훅이 없어도 같은 완료 조건을 지켜요.
...
`cargo test --test r15_lean_orchestration --test r16_live_steering --test r17_orchestration_install`은
실제 `main-runtime` 검사기로 잘못된 역할 경계·반환 누락·입력 처리 규칙을 거절하고,
임시 홈의 양쪽 설치 링크와 출처 표시를 확인해요. `dstack doctor --self`에도 양성·음성
고정물이 등록돼요. 이 검사는 문서의 필수 문장과 알려진 모순을 확인하며, 임의의 문장 의미나
호스트 화면·실제 동시 대화를 증명하지 않아요. 실제 호스트 검증은 별도 실행 기록이 필요해요.
