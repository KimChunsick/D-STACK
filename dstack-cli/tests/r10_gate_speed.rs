// tests/r10_gate_speed.rs
// R10: the Stop hook runs `dstack gate` on every turn, so it has to finish well inside what the
// shell dispatcher needs. The verdict here is relative — half of what the shell takes in the
// same minute, measured call for call next to it — because this machine builds several ports at
// once and a millisecond count taken under load average 8 says nothing about the hook path. The
// 30 ms R10 asks for is still checked: hard on an idle machine, printed as a note on a busy one,
// and proven for real at verification time. The shell gate spawns `check coverage` and `lint-ko`
// as two further dstack processes; the port answers both in process, which is the whole gap.
...
fn measure() -> Measurement {
    ...
    rust.call(&rust_dir, &GATE);
    shell.call(&shell_dir, &GATE);
...
/// The sandbox has an open run with no request.md, so the gate names that one item and exits 1.
/// Both codes it promises are accepted: what is timed is the whole verdict, not a lucky pass.
fn timed(cli: &Cli, dir: &Path) -> Duration {
    let (took, code) = cli.call(dir, &GATE);
    assert!(code == 0 || code == 1, "gate reached a verdict, not {code}");
    took
}
...
#[test]
fn r10_gate_is_fast_enough_for_the_hook_path() {
    let done = measured();
...
#[test]
fn r10_gate_beats_the_shell_by_half() {
    let done = measured();
