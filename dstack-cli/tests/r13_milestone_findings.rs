...
fn record(s: &Scratch, args: &[&str], output: &Output) -> String {
...
fn bundle(s: &Scratch, mid: &str) -> Output {
    ...
    let after = sha256_bytes(s.read(&format!("{RUN}/findings.md")).as_bytes());
    eprintln!("R13 findings SHA256 before: {before}\nafter: {after}");
    assert_eq!(before, after, "original findings hash");
    output
}
fn checked(s: &Scratch, mid: &str, ids: &[&str]) -> String {
    ...
    let report = record(s, &args, &check);
    ...
    assert_eq!(
        frozen,
        format!("{expected}\n"),
        "exact frozen rows in request order"
    );
...
fn R13__both_milestones_keep_only_their_open_findings_and_integration() {
    ...
    for (mid, ids) in [("M1", ["R01", "R02"]), ("M2", ["R03", "R04"])] {
        let text = checked(&s, mid, &ids);
        for (owner, lines) in [
            ("M1", [first, future, r_only]),
            ("M2", [second, no_r, m_only]),
        ] {
            for line in lines {
                assert_eq!(text.contains(line), mid == owner, "{line}");
            }
        }
...
fn R13__followup_coverage_carries_relevant_prior_plan_findings() {
    ...
    assert!([shared, inherited, selected]
        .iter()
...
fn R13__missing_frozen_requirement_is_still_rejected() {
    ...
    let incomplete = REQUEST
        ...
        .collect::<Vec<_>>()
        .join("\n");
    s.write(&format!("{RUN}/request.md"), &incomplete);
...
fn resolution_case(line: &str, closed: bool) {
    let s = fixture(line);
    let text = checked(&s, "M1", &["R01", "R02"]);
    eprintln!("R13 source finding: {line}\nemitted bundle:\n{text}");
    assert_eq!(!text.contains(line), closed, "{line}");
}
#[test]
fn R13__T18_reopened_and_negative_completion_tails_stay_open() {
    for tail in [
        "reopened because the same R01 failure still reproduces.",
        "REOPENED after verification.",
        "not resolved; the failure still reproduces.",
        "no longer fixed.",
        "still open.",
        "remains unresolved.",
        "not verified.",
        ...
        resolution_case(
            &format!(
                "- [P1] R01 was fixed; resolved: commit deadbee (same follow-up commit); {tail}"
            ),
            false,
        );
    }
...
#[test]
fn R13__T18_internal_possessives_preserve_diagnostic_quotes() {
    for (open, close) in [("‘", "’"), ("'", "'"), ("“", "”"), ("\"", "\"")] {
        for possessive in ["users’", "users'", "user’s", "user's"] {
            resolution_case(&format!("- [P1] R01 remains open; diagnostic says {open}{possessive} input; resolved: commit deadbee (same follow-up commit).{close}"), false);
        }
        resolution_case(
            &format!("- [P1] R01 diagnostic says {open}error.{close}; resolved: verified."),
            true,
        );
    }
}
