...
## Maintainer response
Addresses the [T01]-tagged finding (reviewed HEAD not enforced before merge).
- Agreed, fixed. The lifecycle now closes the window end to end: `work` requires the
  worker to leave the tree CLEAN (an unclean tree cannot enter review);
  `before-review` voids fan-out on any uncommitted/untracked change, and the bundle
  binds to recorded base + committed HEAD with both ids in the round file; `merge`
  first verifies the branch HEAD still EQUALS the sealed reviewed HEAD — any commit
  or tree change after sealing reopens the review, declared-scope or not. The
  counterexample (commit src/b after sealing) now fails twice: the pre-merge HEAD
  equality check, and checker scope's clean-tree rule had src/b been left untracked
  at review time.
- Verification: tests/skill-schema.test.sh green (yaml blocks parse; frozen tokens);
  the clean-tree and equality rules are enforced mechanically by check-parallel.sh
  scope (see T02 round 002: unclean tree → VIOLATION test) and by the merge step's
  stated HEAD comparison.

## Carried decisions
- Clean-disjoint-merge integration residual stands as recorded (user decision,
  round 002).
- Per the user's round cap, no further reviewer round: all round-B blocking findings
  for T01 are fixed with mechanical verification recorded above.
