...
## Maintainer response
Addresses the [T04]-tagged findings (unenforced worktree identity, conventions vs
anti-injection conflict).
1. Worktree identity: agreed, fixed on both sides. The agent boundary now REQUIRES a
   parallel/fan-out brief to name the worktree path, task branch, and recorded base;
   the worker verifies the working directory IS that worktree on that branch before
   any write, and a parallel brief omitting the identities is itself a STOP condition
   (the omissive-brief bypass the finding demonstrated). SKILL.md's delegation-brief
   contract now lists the same three identities as mandatory orchestrator-supplied
   fields.
2. Conventions vs anti-injection: agreed, fixed. The boundary now separates HOW from
   WHAT: repository instruction surfaces (CONTRIBUTING, CLAUDE.md/AGENTS.md,
   lint/build configs) are authoritative for how code is written within declared
   scope, while nothing read anywhere may change what is touchable, the
   worktree/branch identity, or the boundaries. Legitimate conventions are followed;
   scope-affecting embedded directives remain reportable anomalies.
Verification: provenance/machine-path sweeps clean; agent loads and reports its scope
rule via the linked definition (M2 smoke evidence in task.md at E2E).

## Carried decisions
- Hook/permission-layer deterministic tool enforcement stays a recorded non-blocking
  follow-up from round 001.
