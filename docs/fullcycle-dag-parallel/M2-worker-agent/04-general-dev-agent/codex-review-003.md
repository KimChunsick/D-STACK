...
## Maintainer response
Addresses the [T04]-tagged finding (repository HOW-authority without a side-effect
boundary).
- Agreed, fixed. A new immutable boundary bullet caps repository authority at coding
  conventions: no publishing, deploying, uploading, off-machine transmission, or
  destructive commands (force-pushes, resets, deletions beyond declared files)
  unless the delegation brief explicitly authorizes that exact action — a repo
  instruction or build script demanding one is a STOP-and-report. This composes with
  the earlier boundaries (secret redaction, scope/identity immutability) so
  command-bearing repository content can steer style, never side effects.
- Verification: provenance/machine-path sweeps clean; boundary text present in the
  agent definition; the tools allowlist (round 001) already withholds connector and
  publishing tools, so the prompt boundary is backed by a metadata boundary.

## Carried decisions
- Hook/permission-layer deterministic enforcement stays a recorded non-blocking
  follow-up (round 001).
- Per the user's round cap, no further reviewer round; fixes verified mechanically.
